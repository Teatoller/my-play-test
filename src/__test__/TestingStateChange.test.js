...
import TestingStateChange from "../components/testStateChange/TestingStateChange";
...
test("Toggle text visible", async () => {
  ...
  // await userEvent.click(screen.getByRole('button', { name: /toggle text/i }));
...
test("Element added to the list", async () => {
    ...
    expect(screen.getByText(/omuga/i)).toBeInTheDocument();
})
